...
\end{pmatrix}
$$
where $\epsilon_{o,e}$ and $\epsilon_{o,g}$ is the environmental and genetic liability of the target, $\epsilon_{p1}$ and $\epsilon_{p2}$ is the full liability of the parents, and $\epsilon_s$ is the full liability of the sibling(s). This is only for one sibling, but can be expanded to an arbitrary number of siblings. The 
...
## Configuration
The configuration that each individual is placed in is based on the phenotype status of the individual itself and its family. In the simulations the column "conf" will represent the configuration that an individual has an will in the form of own phenotypes status, parents phenotypes status and siblings phenotype status, where "2" represents case and "1" represents control. E.g. a configuration of "1212", means that the individual itself is control, while parent 1 is case, parent 2 is control and sibling 1 is case. This is just with one sibling, but can be expanded to an arbitrary number of siblings. Furthermore, a column called "conf_class" will be present after the function ..., which informs the user about the class of the configuration. This is done, since different configurations can be in the same class and therefore be given the same posterior mean genetic liability. The reason behind this is that both parents have the same effect on the the targets genetic liability and the same goes for all the siblings. Therefore, it does not matter which parent is case or control and as long as the family has the same number of siblings, it doesn't matter which siblings are case or control as long as there are just as many case or control in two different configurations, they will be the same. E.g. the configurations "12121" will have the same posterior mean genetic liability as "11212", since the only difference is that the two parents and the two siblings switched phenotype status, which doesn't make a difference to genetic liability as they have the same effect, which is shown in the multivariate normal distribution furhter up. 
